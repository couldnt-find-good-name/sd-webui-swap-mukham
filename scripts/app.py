...
from face_parsing import init_parsing_model, init_parser, swap_regions, mask_regions, get_parsed_mask, mask_regions_to_list, SoftErosion
...
## _______________________________________________ GUI UPDATE FUNC _______________________________________________
    ...
    def swap_process(image_sequence):
        ...
        yield "⌛ Generating faces...", *ui_before()
        preds = []
        matrs = []
        count = 0
        global PREVIEW
        for batch_pred, batch_matr in FACE_SWAPPER.batch_forward(whole_frame_list, analysed_targets, analysed_sources):
            ...
            if USE_CUDA:
                ...
                yield f"⌛ Generating face Batch {count}", *ui_before()

        ## ------------------------------ FACE ENHANCEMENT ------------------------------

        generated_len = len(preds)
        if face_enhancer_name != "NONE":
            yield f"⌛ Upscaling faces with {face_enhancer_name}...", *ui_before()
        ...
        if enable_face_parser:
            yield "⌛ Face-parsing mask...", *ui_before()
            masks = []
            count = 0
            for batch_mask in get_parsed_mask(FACE_PARSER, preds, classes=includes, device=device, batch_size=BATCH_SIZE, softness=int(mask_soft_iterations)):
                ...
                if len(batch_mask) > 1:
                    ...
                    PREVIEW = image_grid[:, :, ::-1]
                    yield f"⌛ Face parsing Batch {count}", *ui_before()
...
urls = [
    ...
    "https://huggingface.co/spaces/ethanNeuralImage/inversion_testing/resolve/6208cab2e6f810b5e59b28bf6850e9a322f6718a/pretrained_models/79999_iter.pth",
                        ...
                        with gr.Accordion("✂️ Video Cut", open=False):
                            with gr.Column():
                                with gr.Row():
                                    ...
                                    show_trim_preview_btn = gr.Checkbox(label="Live Preview", value=True, interactive=True)
